...
function SplashScreen(props) {
  ...
  return (
    <div className={`transition-all duration-350 ease-out absolute top-0 left-0 w-screen h-screen bg-black flex flex-col items-center justify-center gap-16 overflow-hidden ${props.viewSplash ? 'scale-x-100 opacity-100 z-100' : 'scale-x-0 opacity-0 z-0'}`}>
      ...
      <button onClick={props.toggleScreen} className={`transition-all ease-in duration-300 cursor-pointer bg-black text-red-500 shadow-lg hover:shadow-xl shadow-red-500 px-6 py-4 text-3xl font-bold font-[Italiana] rounded-full opacity-0 animate-opacity-4`}>Continue...</button>
    </div>
  )
}
